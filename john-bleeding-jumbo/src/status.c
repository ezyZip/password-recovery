...
#ifdef __EMSCRIPTEN__
/*
 * jtr-wasm live progress.  JtR's normal status reporting is driven by SIGALRM /
 * times(), neither of which works inside a single-threaded WASM synchronous run
 * (signals don't fire mid-callMain; emscripten times() is stuck at 0).  Instead
 * we stream the running candidate count (status.cands) straight to JS.  This is
 * called wherever status.cands grows — chiefly crk_salt_loop() while cracking,
 * and status_update_cands() on the --stdout path.
 *
 * Throttling is essential: for fast formats (millions/sec) an unconditional
 * wasm->JS call would dominate.  We gate clock reads with an adaptive bitmask
 * (widen when candidates arrive fast, narrow when slow) and emit at most ~7
 * times/sec.  emscripten_get_now() (performance.now) advances normally, unlike
 * john's internal clock.  JS computes %/rate/ETA from the raw count.
 */
void status_emit_wasm_progress(void)
{
	static uint64_t mask = 0;        /* only read the clock when (cands & mask)==0 */
	static double last_check = 0, next_emit = 0;
	uint64_t cands = status.cands;

	if (cands & mask)
		return;

	double now = emscripten_get_now();
	double dt = now - last_check;
	last_check = now;
	/* keep clock reads at roughly 250-1000/sec regardless of crack speed */
	if (dt < 1.0 && mask < 0xffff)
		mask = (mask << 1) | 1;
	else if (dt > 4.0 && mask)
		mask >>= 1;
	...
	EM_ASM({
		if (typeof Module !== 'undefined' && Module['onCrackProgress'])
			Module['onCrackProgress']($0);
	}, (double)cands);
}
#endif
