...
/* jtr-wasm: stream the live candidate count (status.cands) to JS. Called where
 * status.cands grows (crk_salt_loop while cracking). Throttled internally. */
extern void status_emit_wasm_progress(void);
#endif
